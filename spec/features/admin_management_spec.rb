...
RSpec.feature "Admin management", :type => :feature, js: false do
  let(:user) { User.first }
  ...
  context "login as guest user" do
    before do
      ...
      unless dist_app.exist?
        dist_app.write("test")
      end
    end

    scenario "Guest user goes to the admin panel" do
      visit root_path
    ...
    scenario "Guest user goes to collections page" do
      login_as user
    ...
    scenario "Guest user goes to slides page" do
      login_as user
      visit slides_path
      expect(page).not_to have_text "Admin Panel"
      expect(page).to have_text("You do not have sufficient permissions to view this page")
    ...
    scenario "Guest user goes to kiosks page" do
      login_as user
      visit slides_path
      expect(page).not_to have_text "Admin Panel"
      expect(page).to have_text("You do not have sufficient permissions to view this page")
  ...
  context "login as admin user" do
    ...
    scenario "User goes to the admin panel" do
      login_as user
      visit root_path
    ...
    scenario "User goes to the list of kiosk layouts page" do
      login_as user
      visit root_path
      click_link "Admin Panel"
    ...
    scenario "User goes to the list of slide types page" do
      login_as user
      visit root_path
      click_link "Admin Panel"
      within("body.rails_admin .content") do
    ...
    scenario "User goes to the list of users page" do
      login_as user
      visit root_path
      click_link "Admin Panel"
      within("body.rails_admin .content") do
