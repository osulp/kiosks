...
import {
  ...
  SET_SLIDES,
  SET_RESTART_KIOSK
} from "../../../../app/javascript/react/actions/kioskActions"
import {
  FETCHING_SLIDES,
  FETCHED_SLIDES,
  FETCHING_RESTART_KIOSK,
  FETCHED_RESTART_KIOSK,
  FETCHING_HOURS,
  FETCHED_HOURS,
  SET_HOURS
...
describe("Action::Touch", () => {
  describe("#fetchSlides()", () => {
    ...
    it("should return an error", () => {
      ...
      const nock_error_message = `request to ${bogus_host}${bogus_action_json} failed, reason: ${
      ...
      const expectedActions = [
        ...
        {
          type: ADD_ERROR,
          error: { message: nock_error_message, code: factories.error.code }
        },
        { type: FETCHED_SLIDES }
      ]
      const store = mockStore({ slides: [] })
  ...
  describe("#fetchHours()", () => {
    ...
    it("should return an error", () => {
      ...
      const nock_error_message = `request to ${bogus_host}${bogus_api_hours} failed, reason: ${
        factories.error.message
      }`
      // The actions that were set in the store during the async call.
      // If an async call dispatches multiple actions, they must be set here.
    ...
    it("should return a 404 error", () => {
      ...
      const expectedActions = [
        ...
        {
          type: SET_HOURS,
          hours: { error: "Hours not found for the selected date." }
        }
      ]
      const store = mockStore({ hours: [] })
    ...
    it("should return a 500 error", () => {
      ...
      const expectedActions = [
        ...
        {
          type: SET_HOURS,
          hours: { error: "Error fetching hours, please notify the help desk." }
        }
      ]
      const store = mockStore({ hours: [] })
