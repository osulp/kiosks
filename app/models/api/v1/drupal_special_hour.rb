...
module Api
  module V1
    class DrupalSpecialHour < DrupalRecord
      self.table_name = APPLICATION_CONFIG['api']['database']['drupal']['special_hours']['table_name']

      # Query Drupal to find the special times the Library is open and closed for an array of Dates.
      ...
      def self.hours_for_dates(dates)
        ...
        dates.map! { |d| d.to_date }.sort.each do |date|
          ...
          result[parsed_time] = { open: open_time.strftime(APPLICATION_CONFIG['api']['hours']['hours_for_dates']['open_close_time_format']),
                                  ...
                                  formatted_hours: Api::V1::CustomHoursFormat.new({open_time: open_time, close_time:close_time}).formatted_hours
          }
        end
        return result
      end
