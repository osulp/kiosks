...
class Header extends Component {
  ...
  mapsClicked() {
    ...
    let tabs = this.props.maps.map(m => {
      return {
        button_text: m.title,
        content: (
          <div style={{ backgroundImage: `url("${m.image_url}")` }}>&nbsp;</div>
        )
      }
    })
    this.props.setModalVisibility(true)
    this.props.setModalRootComponent(
      <ConnectedTabbedPanel
        id="maps_tabbed_panel"
        tabs={tabs}
        selectedIndex={1}
        timeout={30000}
      />
   ...
   */
  _hoursToday() {
    if (this.props.todays_hours == null || Object.values(this.props.hours).length == 0 || Object.values(this.props.hours) == undefined) {
      return ""
    } else {
      let today = Object.values(this.props.hours)[0]
      return `${today.formatted_hours_plain_text} on ${today.string_date}`
    }
  }
            ...
            <div className="container">
              <div className="navbar-header">
                ...
                <img
                  ...
                  src="/images/osulibrarylogo.png"
                  onClick={this._didTap.bind(this)}
                />
              </div>
              <div id="navbar" className="navbar-collapse collapse">
                <ul className="nav navbar-nav">
                  <li
                    ...
                    onClick={this.hoursClicked.bind(this)}
                  >
                ...
                <ul className="nav navbar-nav navbar-right">
                  <li
                    className="refresh-slides"
                    onClick={this.refreshClicked.bind(this)}
                  >
                    <a>
                      <span
