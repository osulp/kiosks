...
class ClassroomSchedule extends Component {
  ...
  classroomClicked(classroom) {
    trackClicked(
      ...
      `${this.props.kiosk_name}:${
        this.props.kiosk_id
      }:InteractiveKiosk:ClassroomSchedule:Classroom`
    )
  ...
  dateClicked(date) {
    trackClicked(
      ...
      `${this.props.kiosk_name}:${
        this.props.kiosk_id
      }:InteractiveKiosk:ClassroomSchedule:Date`
    )
  ...
  componentDidMount() {
    ...
    this.props.fetchClassroomSchedule(
      this.props.api.classroom_schedule,
  ...
  _sortedClassrooms() {
    const ordered = []
    Object.keys(this.props.classrooms)
      .sort()
      .forEach(k => ordered.push(this.props.classrooms[k]))
    return ordered
  }
  ...
  render() {
    ...
    const is_fetching_class = this.props.is_fetching_classroom_schedule
      ? "is_fetching"
      : ""
    ...
    return (
      <div id="classroom_schedule" className="panel panel-default">
        <div className="panel-heading">
          ...
          <span
            className={`glyphicon glyphicon-repeat ${is_fetching_class}`}
            aria-hidden="true"
          >
            &nbsp;
          </span>
            ...
            <div className="classroom-schedule-table-datepicker hidden-md hidden-lg col-sm-12">
              ...
              >
                {({ value }) => {
                  return (
                    <span tabIndex="0">
                      <label htmlFor="classroom_schedule_date_picker">
                        Select a Date:
                      </label>
                      <input
                        id="classroom_schedule_date_picker"
            ...
            <div className="hidden-sm hidden-xs col-md-12 classroom-filter">
              <ul>
                ...
                {classrooms.map(classroom => {
                  let selected = classrooms.some(this._hasSelectedClassroom)
                    ? classroom.selected
                    : true
                  return (
                    <li
                      key={classroom.shortname}
                      className={`${classroom.shortname} ${
