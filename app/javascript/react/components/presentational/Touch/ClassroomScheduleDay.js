...
class ClassroomScheduleDay extends Component {
  ...
  _eventDetailRow(event, i) {
    return (
      <tr
        key={`event.${i}.detail`}
        className={`event-detail-row ${event.room_shortname}`}
      >
  ...
  _eventTimeRow(date, event, i, willamette) {
    ...
    return (
      <tr
        key={`event.${i}.time`}
        className={`event-time-row ${event.room_shortname} ${
          event.combined_willamette_room ? "willamette-combined" : ""
        } ${willamette ? "willamette-selected" : ""}`}
      >
        {CLASSROOM_HOURS.map(h => {
          const hour = moment(date).hour(h)
          const has_hour = this._eventIncludesTime(start_time, end_time, hour)
          const has_15 = this._eventIncludesTime(
            start_time,
            end_time,
            moment(hour).add(15, "m")
          )
          const has_30 = this._eventIncludesTime(
            start_time,
            end_time,
            moment(hour).add(30, "m")
          )
          const has_45 = this._eventIncludesTime(
            start_time,
            end_time,
            moment(hour).add(45, "m")
          )
          const hour_has_event = has_hour || has_15 || has_30 || has_45
          return (
            <td
              key={`event.${i}.${hour}`}
              className={hour_has_event ? "event-hour" : ""}
            >
              <span
                key={`event.${i}.hour.header`}
                className="event-hour-header"
              >
                {hour.format("h a")}
              </span>
              <span
                key={`event.${i}.00`}
                className={has_hour ? "event-time" : ""}
              >
                &nbsp;
              </span>
              <span
                key={`event.${i}.15`}
                className={has_15 ? "event-time" : ""}
              >
                &nbsp;
              </span>
              <span
                key={`event.${i}.30`}
                className={has_30 ? "event-time" : ""}
              >
                &nbsp;
              </span>
              <span
                key={`event.${i}.45`}
                className={has_45 ? "event-time" : ""}
              >
                &nbsp;
              </span>
            </td>
  ...
  _willametteEastWestSelected(selected) {
    const willamette_west_selected = selected.filter(function(selection) {
      return selection.shortname === "lib-willwest"
    })
    const willamette_east_selected = selected.filter(function(selection) {
      return selection.shortname === "lib-willeast"
    })
    return (
      willamette_west_selected.length > 0 && willamette_east_selected.length > 0
    )
  }
  ...
  _getEventRows(classrooms, events, date) {
    if (!events || !events.length) {
      ...
      this._filteredEvents(classrooms, events).map((e, i) => {
        const selected = Object.values(classrooms).filter(
          c => c.selected == true
        )
  ...
  _filteredEvents(classrooms, events) {
    const some_selected = Object.values(classrooms).some(
      c => c.selected === true
    )
    const selected = some_selected
      ? Object.values(classrooms).filter(c => c.selected == true)
    ...
    if (this._willametteEastWestSelected(selected) === true) {
      ...
      return Object.values(will_combined).filter(e =>
        selected.some(s => s.shortname == e.room_shortname)
      )
    }
  ...
  render() {
    ...
    return (
      <table className="table classroom-schedule-table">
        ...
        <tbody>{this._getEventRows(classrooms, events, date)}</tbody>
      </table>
    )
  }
}
