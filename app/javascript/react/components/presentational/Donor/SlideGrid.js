...
class SlideGrid extends Component {
  ...
  setRandomTimer() {
    const randomTimer = () => {
      ...
      let randomNumber = Math.floor(
        (Math.random() * 1000000) % Object.keys(filtered_slides).length
      )
      this.props.setModalRootComponent(
        <LargeSlide
          slide={filtered_slides[randomNumber]}
  ...
  filteredSlides(filter_status) {
    if (this.props.title == "") {
      ...
      return this.props.slides.filter(
        slide => slide.slide_type == this.props.title
      )
    }
  }
  ...
  setTitle(e) {
    trackClicked(
      ...
      `${this.props.kiosk_name}:${
        this.props.kiosk_id
      }:DonorKiosk:FilterSlides:${e.target.dataset.slidetype}`
    )
    clearTimeout(this.random_timer)
    ...
    return (
      <div className="container-fluid">
        <div className="row">
          ...
          <div className="col-md-4 donor-title text-center">{title}</div>
          <div className="col-md-4" />
        </div>
